...
fig.suptitle('DEV11 WebKit: the status wait maps to shader program 45', x=.14, y=.965, ha='left', fontsize=17, weight='bold')
fig.text(.14, .922, 'One 60.01540 s diagnostic · unchanged original package · engine-loop cadence, not presented FPS', fontsize=11)
ax.plot(x, y, color='#8492a6', linewidth=.65, alpha=.9)
ax.axhline(20, color='#90a5b7', linewidth=.8, linestyle='--')
ax.set(xlim=(0, 60), ylim=(0, 650), ylabel='Start-to-start interval (ms)', xlabel='Seconds from first measured callback')
ax.grid(axis='y', alpha=.15)
for h, offset in zip(review['hitches'], [(3, 612), (36, 325)]):
...
for name, color, value in series:
    ...
    bars.barh(range(len(review['hitches'])), values, left=left, color=color, height=.58, label=name)
...
bars.set_yticks(range(len(review['hitches'])), [f"{h['relative_start_s']:.5f} s\ncallback {h['callback_index']}" for h in review['hitches']])
bars.invert_yaxis()
bars.set(xlim=(0, 650), xlabel='Milliseconds within each start-to-start interval')
...
fig.text(.14, .012, 'Program 45: lit USE_ATTRIBUTES variant of lit_visible_pixels · 90 original source versions · 0 drops/errors\nStatus-query wall time is not exclusive GPU or compiler-only time. The second hitch remains unattributed.', fontsize=10, color='#445264')
