...

{
  ...
  const ready=plain(f.api.ready());assert.equal(ready.error_mask,0);assert.equal(ready.installed,9);
  ...
  ALL_METHODS.forEach((name,i)=>{assert.equal(f.gl[name],f.native[name]);assert.deepEqual(Object.getOwnPropertyDescriptor(f.gl,name),f.nativeDescriptors[i]);});
...
}
{
  ...
  const identities=analyzeIdentity(capture);assert.equal(identities.timed_program_status.length,0);assert.equal(identities.program_count,1);
