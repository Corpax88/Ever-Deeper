...
def main(root):
    ...
    prep, request = [read(root/n) for n in ['preparation-remote.json', 'request-remote.json']]
    expected = {Path(name).name: value['sha256'] for name, value in prep['manifest'].items() if name.startswith('tools/')}
    expected['REQUEST.json'] = request['request_sha256']
    assert host['source_hashes'] == expected and host['study_sha'] == request['request_commit']
    assert host['request']['preparation_sha'] == prep['preparation_sha'] and host['request']['sessions'] == 1
    ...
    assert run['head_sha'] == request['request_commit'] and run['run_attempt'] == 1 and run['conclusion'] == 'success'
    ...
    assert log.count('WEBKIT_SHADER_IDENTITY_COMPLETE ') == 1 and 'WEBKIT_SHADER_IDENTITY_FAILED ' not in log
    ...
    assert rr['method_count'] == 9 and rr['error_mask'] == 0
    ...
    review = {'schema': 1, 'run_id': run['id'], 'measured_request_sha': request['request_commit'], 'preparation_sha': prep['preparation_sha'],
              ...
              'limits': ['Overhead-bearing single diagnostic with a different random seed and runner conditions; no regression or improvement comparison.',
                         ...
                         'The second major callback and all unwrapped time remain unattributed.',
                         'Original shader/program associations are captured separately; exact depth or draw owner at each hitch was not recorded, so biome and precise CanvasItem causality remain unknown.',
