...
const BRANCH='refs/heads/codex/dev11-webkit-shader-identity-20260917';
...
try {
  ...
  if(request.sessions!==1||request.source!==SOURCE||request.diagnostic_only!==true||request.probe!=='selected_webgl_shader_identity_v1'||!request.preparation_sha)throw Error('Missing one-session authorization');
  ...
  if(requestDiff.trim()!=='A\ttools/webkit_shader_identity/REQUEST.json')throw Error('Request child changes more than its new request file');
  ...
  console.log((result.complete?'WEBKIT_SHADER_IDENTITY_COMPLETE':'WEBKIT_SHADER_IDENTITY_FAILED')+' '+JSON.stringify({source:SOURCE,pck:result.pck_sha256,complete:result.complete,metric:result.metric,error:result.error||null}));
}
