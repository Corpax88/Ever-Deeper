...
var debug_spans: Array[Rect2] = []
var contour_usec: int = 0
...
func refresh() -> void:
	...
	debug_spans = spans.duplicate()
	var begun: int = Time.get_ticks_usec()
	var polygons: Array[PackedVector2Array] = Contours.from_spans(spans, tile)
	contour_usec = Time.get_ticks_usec() - begun
	while _pool.size() < polygons.size():
		var occluder: LightOccluder2D = LightOccluder2D.new()
		occluder.occluder = OccluderPolygon2D.new()
		_pool.append(occluder)
		add_child(occluder)
	active_count = polygons.size()
	for i in _pool.size():
		_pool[i].visible = i < polygons.size()
		if i >= polygons.size():
			continue
		_pool[i].occluder.polygon = polygons[i]
