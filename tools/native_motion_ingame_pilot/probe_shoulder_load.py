"""One original-rig backswing proposal, dense checks and seven native stills.
...
for name in ('native-tools', 'pivot-report', 'cap-proof', 'recorded', 'output'):
...
source_hashes = dict(cap_proof['source_hashes'])
for relative, digest in source_hashes.items():
    assert sha(ROOT / relative) == digest, relative
for name in ('shoulder_load_motion.py', 'probe_shoulder_load.py'):
...
motion = ShoulderLoadMotion(surface, hinge, pivot)
env['view']('up', (6, 6))
...
report = {'complete': False, 'passed_geometry': False, 'rendered': False,
          ...
          'inputs': {str(p): sha(p) for p in (args.pivot_report, args.cap_proof, args.recorded)},
...
try:
    ...
    for q in phases:
        ...
        assert candidate['maximum_arm_reach'] < .71, row
    ...
    save('shoulder-load-progress.json')
    ...
    save('shoulder-load-final.json')
    print('SHOULDER_LOAD_COMPLETE', len(report['samples']), len(report['renders']), flush=True)
except Exception as error:
    report.update(complete=True, failure=str(error), failure_phase=q, traceback=traceback.format_exc())
    save('shoulder-load-rejected.json')
    raise
