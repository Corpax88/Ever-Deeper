...
try:
    for q in sorted({native_phase(i/400) for i in range(401)}|{.40,.50,.55,.625,.70}):
        ...
        cap_error=(cap(before)-cap(after)).length
        ...
        r['samples'].append({'phase':q,'cap_error':cap_error,'protected_error':max(body_error,legs_error,feet_error),'arm_length_error':lengths,'max_arm_reach':reach})
