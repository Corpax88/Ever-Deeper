"""One last-pre-hit cancellation from the actually shown cross-shoulder pose.
...
try:
    ...
    report.update({'complete': False, 'passed_geometry': True, 'rendered': False, 'source_sha': subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=ROOT, text=True).strip(),
              ...
              'scope': 'One late cancel from captured sample49, last real draw before damage50. Dense native cap samples against frozen draw49 ore alpha; '
    ...
    for label, t in [('source', 0.), ('middle', .05), ('next-draw', 1 / 60), ('idle-end', clip.duration)]:
        ...
        print('LATE_WINDUP_CANCEL_FRAME', label, flush=True)
    ...
    with (args.output / 'late-windup-cancel-final.json').open('w') as f:
        json.dump(report, f, indent=2)
        f.write('\n')
        f.flush()
        os.fsync(f.fileno())
    print('LATE_WINDUP_CANCEL_COMPLETE', report['projected_clear_at_sampled_points'], flush=True)
    ...
    with (args.output / 'late-windup-cancel-rejected.json').open('w') as f:
