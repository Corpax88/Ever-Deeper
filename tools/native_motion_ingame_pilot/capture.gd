...
const DIRECTIONS := {"right": Vector2.RIGHT, "up": Vector2.UP}
var output := ""
var source_sha := ""
var mode := "geometry"
var replay_path := ""
...
func _candidate_sequence() -> void:
	...
	for frame in 50:
		...
		var shown: Dictionary = player.visual.presented_snapshot()
		if not bool(shown.is_bridge) and shown.state == "mine" and is_equal_approx(float(shown.sample_phase), 0.625):
			recovered = true
			break
	if not _check(recovered and int(player._mining_impact_serial) == initial_serial + 1, "Exact presented mine .625 source follows one actual strike"): return
	...
	for frame in 50:
		...
		if (Vector2(player.global_position) - exit_origin).dot(direction) >= EXIT - 0.01:
			exited = true
			break
	if not _check(exited and not bool(world.mining_active) and int(player._mining_impact_serial) == initial_serial + 1, "Immediate normal same-heading exit cancels mining without another hit"): return
	var bridge_starts := 0
	var handoffs := 0
	...
	_check(impacts == 1, "One actual drawn native .55 contact accompanies the real impact")
...
func _verify_graphic_motion() -> void:
	...
	_check(movement_frames >= 40, "Actual approach and exit contain complete gait observation intervals", movement_frames)
