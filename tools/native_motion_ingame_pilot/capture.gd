...
const BRIDGE_RESTART_ASSETS := "res://tools/native_motion_ingame_pilot/assets/worn-bridge-restart/"
const BRIDGE_RESTART_ASSET_HASHES := {
	"manifest.json": "8e1f34e625e412cfdd6dae402389727d1d4ea148d8437f7271dc8953c6954296",
	"up-cloth.png": "d3a5437e1881ebe69529d5875b226b9fe834d1787b3396d29004fde0406abb87",
	"up-p001-cloth.png": "5534c4468837153b7ed5d061004d86028fee424c0578ae5a7ca4d1e1ca1abe44",
	"up-p001.png": "d786d9bbfdbb69372a25d02f60d7eed3eca45e0b8ef322e63e0171f57e45a81e",
	"up.png": "adb7ecfdb72a564c4e125b580953d12b38e15b0683bdaf0a68596e10885a766b"
}
const CROSS_SHOULDER_ASSETS := "res://tools/native_motion_ingame_pilot/assets/worn-cross-shoulder/"
const CROSS_SHOULDER_ASSET_HASHES := {
	"manifest.json": "e4f382b24a4e85e1a56ef2ac3ee172e70f95b2ddaee16c381f5362f66be71185",
	"up-cloth.png": "f9f7064448fad7d36bc6cb86669dbecb5cc72c211fb91c5dec00c43ba03303ee",
	"up-p001-cloth.png": "6d02f1df8c25c59f82e925051d7ff1ec611dba70b4f654f0e26df9eb78127745",
...
const COMPLETE_RETURN_ASSETS := "res://tools/native_motion_ingame_pilot/assets/worn-complete-return/"
const COMPLETE_RETURN_ASSET_HASHES := {
	"manifest.json": "e2fcf1275bc80c88fa5ee5c1768154445e355ea587db58d80032ec50efa09516",
	"up-cloth.png": "8b48372855b42c6b3d389bd3189008fc072909a3b32f66c23c149c47ae5394f8",
	"up-p001-cloth.png": "a100d539686de7255c68ca43207e12fca19b5edaca689ee3baf3bdf008aba7a6",
	"up-p001.png": "75681e3c5e9de3cd0c1aa134f622b0c662e45c738deba32a002779070b3dd69b",
	"up.png": "dbf8a5d322a7f09338e972fd72657347a3879d1f7fe341d05b20e350d21a04e1"
}
...
var grounded_entry_probe := false
var grounded_exit_probe := false
...
func _run() -> void:
	for arg in OS.get_cmdline_user_args():
		...
		elif arg == "--grounded-entry-probe": grounded_entry_probe = true
		elif arg == "--grounded-exit-probe": grounded_exit_probe = true
		elif arg == "--late-cancel-cycle":
			late_cancel_cycle = true
			cancel_cycle = true
		elif arg == "--continue-framing-diagnostics": continue_framing_diagnostics = true
	if (grounded_exit_probe and not grounded_entry_probe) or (grounded_entry_probe and not loop_flow_probe) or startup_wall_seconds < 1 or startup_wall_seconds > 900 or not output.is_absolute_path() or not DIRECTIONS.has(direction_name) or source_sha.length() != 40 or mode not in ["geometry", "candidate", "baseline"] or (mode != "geometry" and DisplayServer.get_name() == "headless") or (int(rest_cycle) + int(cancel_cycle) + int(bridge_restart_cycle) > 1):
		print("NATIVE_INGAME_USAGE --mode=geometry|candidate|baseline --direction=up --source-sha=<40hex> --output=<absolute> [--depth=1] [--replay=<candidate report>]; visual modes require a rendered display")
		quit(2)
		return
	...
	if bridge_restart_cycle:
		asset_root = BRIDGE_RESTART_ASSETS
		asset_hashes = BRIDGE_RESTART_ASSET_HASHES
	...
	if loop_flow_probe:
		...
		var identity: Variant = JSON.parse_string(FileAccess.get_file_as_string(asset_root + "identity.json"))
		var expected_cells := 91 if grounded_exit_probe else 83 if grounded_entry_probe else 76
		if not _check(identity is Dictionary and identity.get("loop_flow_study", false) and int(identity.get("angle_degrees", -1)) == 0 and identity.get("rendered_cells", []).size() == expected_cells and bool(identity.get("grounded_entry_study", false)) == grounded_entry_probe, "Loop flow identity has the exact original-view route cells", {"expected": expected_cells, "grounded_entry": grounded_entry_probe}):
			_finish()
			return
		if grounded_entry_probe:
			if not _check(identity.get("source_sha", "") == source_sha and identity.get("reference_report_sha256", "") == "65528376612e44c37311775b1df1833e900c5a5a4b3aa1d84fda6f807f733ad0" and identity.get("replaced_clip", "") == "walk_to_mine-625000" and is_equal_approx(float(identity.get("entry_duration", 0.0)), 0.20770909090909093), "Grounded entry is bound to the recorded119-frame route and original hit clock"):
				_finish()
				return
		if grounded_exit_probe:
			if not _check(bool(identity.get("grounded_exit_study", false)) and identity.get("contact_exit_reference_report_sha256", "") == "d0a9b21b7159f6d1540b147ec5dd7f754df3ebf36684a1514d00167d54c2b61c" and is_equal_approx(float(identity.get("exit_duration", 0.0)), 0.22) and is_equal_approx(float(identity.get("lower_exit_duration", 0.0)), 0.08380545880345398), "Grounded exit preserves the original lower clock on the91cell route"):
				_finish()
				return
		asset_hashes = identity.asset_hashes
		view_angle_cells = identity.rendered_cells
	if mode == "baseline":
		...
		var same_runtime: bool = replay.get("production_runtime_source", "") == BASE_SOURCE and replay.get("consumer_sha256", "") == FileAccess.get_sha256("res://tools/native_motion_ingame_pilot/pilot_visual.gd") and replay.get("asset_hashes", {}) == asset_hashes and replay.get("runtime_world_sha256", "") == FileAccess.get_sha256("res://scripts/world/endless_descent_world.gd")
...
func _candidate_sequence() -> void:
	...
	if cancel_cycle or bridge_restart_cycle:
...
func _cancel_and_restart() -> void:
	...
	for frame in 15:
		...
		var shown: Dictionary = player.visual.presented_snapshot()
		var selected_source: bool = shown.state == "mine_to_idle-392857" and bool(shown.is_bridge) and is_equal_approx(float(shown.sample_phase), 0.416666666666667) if bridge_restart_cycle else shown.state == "idle" and not bool(shown.is_bridge) and is_equal_approx(float(shown.sample_phase), 0.0333333333333333)
		if selected_source:
			ready = true
			break
	if not _check(ready, "Restart source is the actual 50 ms stop-bridge draw" if bridge_restart_cycle else "The first actually presented idle .033333 enables the authored restart"): return
	# Input resumes immediately after the selected actual draw. An active-bridge
	# restart must not wait for idle or reuse the old mechanical mine clock.
	...
	for event in player.visual.transitions:
		...
		if event.event == "canonical_handoff": handoffs += 1
		if event.event == "bridge_interrupted": interruptions += 1
	_check(starts == 5 and handoffs == (4 if bridge_restart_cycle else 5) and interruptions == (1 if bridge_restart_cycle else 0), "Every requested bridge either completes or has its declared interruption", {"starts": starts, "handoffs": handoffs, "interruptions": interruptions})
	var impacts := 0
	for sample in samples:
		if bool(sample.visual.get("presenting_impact", false)) and is_equal_approx(float(sample.visual.get("sample_phase", -1.0)), 0.55): impacts += 1
...
func _verify_graphic_motion() -> void:
	...
	_check(movement_frames >= (24 if cancel_cycle or bridge_restart_cycle else 40), "Actual route contains its complete required gait observation intervals", {"frames": movement_frames, "approach_only": cancel_cycle or bridge_restart_cycle})
...
func _record(label: String) -> void:
	...
	samples.back()["resource_hit_presentation"] = world.resource_hit_presentation_snapshot(String(route.resource_id))
