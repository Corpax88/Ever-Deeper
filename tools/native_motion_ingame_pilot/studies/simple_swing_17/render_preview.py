...
def apply(p):
    ...
    return dict(reach=reach,grip=grip,arm_quaternions={n:list(rig.pose.bones[n].matrix.to_quaternion()) for n in reference})
...
try:
    ...
    times=[i/50 for i in range(50)] if args.loop else [k[0] for k in motion.keys[:-1]]
    ...
    print('SIMPLE_SWING_COMPLETE',len(report['renders']),flush=True)
