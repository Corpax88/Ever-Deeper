...
func _draw_frame(delta: float) -> void:
	...
	if moving and _mode == "mine" and _walk_atlas != null:
		...
		if study_frame == 31 and direction_name == "down" and active_gear == "worn" and absf(_walk_phase-340.0/60.0/144.0)<.0001:
			_walk_active = true
			_walk_elapsed = 0.0
			_mode = "walk_bridge"
		else:
	...
	if _walk_active:
		_walk_elapsed += delta
		if moving and direction_name == "down" and _walk_elapsed <= .200001:
			...
			return
		_walk_active = false
	if moving or direction_name != "up" or active_gear != "worn":
		_mode = "legacy"
		_restart_source = -1.0
		study_impact = false
		super._draw_frame(delta)
