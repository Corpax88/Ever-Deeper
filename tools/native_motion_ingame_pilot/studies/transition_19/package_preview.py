...
if a.video or a.gif:
    ...
    input=['-framerate','60','-i',str(a.game/'crops/frame-%04d.png')]
    if a.video:
        a.video.parent.mkdir(parents=True,exist_ok=True)
        subprocess.run(['ffmpeg','-hide_banner','-loglevel','error','-y',*input,
            '-vf','scale=640:560:flags=lanczos','-an','-c:v','libx264','-crf','18',
            '-pix_fmt','yuv420p','-movflags','+faststart',str(a.video)],check=True)
        subprocess.run(['ffmpeg','-v','error','-i',str(a.video),'-f','null','-'],check=True)
    if a.gif:
        subprocess.run(['ffmpeg','-hide_banner','-loglevel','error','-y',*input,
            '-vf','fps=30,scale=480:420:flags=lanczos,split[a][b];[a]palettegen[p];[b][p]paletteuse=dither=sierra2_4a',
            '-loop','0',str(a.gif)],check=True)
