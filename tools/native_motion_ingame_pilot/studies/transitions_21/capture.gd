...
func _run() -> void:
	var source_hashes := {}
	for path in ["res://tools/native_motion_ingame_pilot/studies/transitions_21/capture.gd", "res://tools/native_motion_ingame_pilot/studies/transition_19/transition_visual.gd"]:
	...
	for arg in OS.get_cmdline_user_args():
		...
		if arg == "--candidate21": candidate21 = true
		if arg.begins_with("--bank="): bank = arg.trim_prefix("--bank=")
	...
	main.get_node("EndlessDescentWorld/Player/Visual").set_script(load("res://tools/native_motion_ingame_pilot/studies/transitions_21/transition_visual.gd" if candidate21 else "res://tools/native_motion_ingame_pilot/studies/transition_19/transition_visual.gd"))
	root.add_child(main)
	current_scene = main
	...
	if candidate21: player.visual.configure_bank(bank)
	...
	if scenario == "walk-entry":
		events = {0:"walk_up",8:"walk_to_mine",70:"release_after_hit"}
		total = 106
	...
	for i in total:
		...
		sample["transition"] = player.visual.transition_snapshot()
		sample["walk_phase"] = player.visual._walk_phase
		sample["idle_clock"] = player.visual._idle_clock
	...
	var report := {"complete":true,"samples":samples,"initial_hp":initial_hp,"damage":damage,"hits":hits,
		"scenario":scenario,"candidate21":candidate21,
		...
		"viewport":[root.size.x,root.size.y],"mechanical_pass":hits>=1 and damage==hits*4,
	...
	file.close()
	print("TRANSITION21_COMPLETE ",hits," ",damage)
