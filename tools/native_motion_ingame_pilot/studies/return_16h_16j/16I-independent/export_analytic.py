...
TEMP = Path('/tmp/pivot-return-16I-plan')
BODY_PLAN = Path('/tmp/pivot-return-16E-plan')
TOOL_PLAN = Path('/tmp/pivot-return-16D-plan')
EXPECTED = '1fc51407bd69b90b993a688be9aa631868ffae1af134f21f33f320864a86aaff'
sha = lambda p: hashlib.sha256(Path(p).read_bytes()).hexdigest()
OUT.mkdir(parents=True, exist_ok=True)
assert sha(TEMP / 'pivot_return_motion.py') == EXPECTED
assert sha(TEMP / 'transported_forearm_frame_pose.py') == '11fb4baa0366afeedca66c34b7a108187f5c2cc3b977df8cbdb2766ab14d4982'
...
def add(prefix, p):
    ...
    values['has_extra_arm_transports'] = 'extra_arm_transports' in p
    for name in ('upper.R', 'lower.L'):
        key = name.replace('.', '_')
        values['extra_'+key+'_transport'] = [list(row) for row in p.get('extra_arm_transports', {}).get(name, np.eye(3))]
        values['extra_'+key+'_original_axis'] = list(p.get('extra_arm_original_axes', {}).get(name, (0.,0.,0.)))
        values['extra_'+key+'_reference_denominator'] = p.get('extra_arm_reference_denominators', {}).get(name, 0.)
