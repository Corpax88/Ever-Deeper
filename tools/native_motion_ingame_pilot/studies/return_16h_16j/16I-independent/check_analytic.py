"""Independent16I full-frame gate; fixed G rotations plus declared translation.
...
OUT = Path('/tmp/pivot-return-16I-independent')
...
assert meta['candidate_sha256']=='1fc51407bd69b90b993a688be9aa631868ffae1af134f21f33f320864a86aaff'
assert meta['frame_helper_sha256']==sha(OUT/'inputs'/'transported_forearm_frame_pose.py')=='11fb4baa0366afeedca66c34b7a108187f5c2cc3b977df8cbdb2766ab14d4982'
...
assert constants['TRANSLATION_FIT_SHA256']=='f3b1fecd61a4d81c02f118611f8c84e413a39d56e4f4ba157c7cf8ebd1ac8707'
...
def frames(prefix):
    ...
    for side, j in [('R', 0), ('L', 1)]:
        ...
        if side == 'R':
            ...
            result['lower.L'] = base
    extra = d[prefix+'_has_extra_arm_transports'].astype(bool)
    for bone in ('upper.R','lower.L'):
        key=bone.replace('.','_')
        prior,_=shortest(meta['rest_axes'][bone],d[prefix+'_extra_'+key+'_original_axis'][extra])
        result[bone][extra]=d[prefix+'_extra_'+key+'_transport'][extra] @ prior
...
extra_explicit=d['new_has_extra_arm_transports'].astype(bool)
assert np.array_equal(extra_explicit,explicit)
assert not d['old_has_extra_arm_transports'].any()
extra_report={};extra_checks={}
for bone,side,part in [('upper.R',0,0),('lower.L',1,1)]:
    key=bone.replace('.','_')
    original_axis=unit(d['old_arms'][:,side,part+1]-d['old_arms'][:,side,part])
    actual_axis=unit(d['new_arms'][:,side,part+1]-d['new_arms'][:,side,part])
    carried=np.einsum('nij,nj->ni',body_rotation,original_axis)
    align,den=shortest(carried,actual_axis)
    planned_transport=align@body_rotation
    exported=d['new_extra_'+key+'_transport'][extra_explicit]
    checks=dict(
        original_axis=float(np.max(abs(d['new_extra_'+key+'_original_axis'][extra_explicit]-original_axis[extra_explicit]))),
        transport=float(np.max(abs(exported-planned_transport[extra_explicit]))),
        axis=float(np.max(abs(np.einsum('nij,nj->ni',exported,original_axis[extra_explicit])-actual_axis[extra_explicit]))),
        orthogonality=float(np.max(abs(exported.transpose(0,2,1)@exported-np.eye(3)))),
        determinant=float(np.max(abs(np.linalg.det(exported)-1))),
        reported_denominator=float(np.max(abs(d['new_extra_'+key+'_reference_denominator'][extra_explicit]-den[extra_explicit]))),
        complete_frame=float(np.max(abs(new[bone][extra_explicit]-(planned_transport@old[bone])[extra_explicit]))),
        protected_fallback_frame=float(np.max(abs(new[bone][protected]-old[bone][protected]))))
    extra_report[bone]=dict(errors=checks,original_rest_minimum_denominator=minimum_row(oldden[bone]),transport_minimum_denominator=minimum_row(den),explicit_samples=int(extra_explicit.sum()))
    extra_checks['declared_extra_'+key]=max(checks.values())<1e-5
    extra_checks['original_extra_rest_'+key]=bool(oldden[bone].min()>.01)
    extra_checks['extra_transport_guard_'+key]=bool(den.min()>.05)
extra_checks['protected_extra_fallback']=bool(not extra_explicit[protected].any())
basic_checks = dict(independent_curves=max(curve_errors.values()) < 1e-7,finite_data=all(np.isfinite(d[k]).all() for k in d.files),reach=bool(reach.max() < .70),segment_lengths=length_error < 1e-5,
                    ...
                    native_axis_maps=bool(all(newden[k][~extra_explicit].min()>.01 for k in ('upper.R','lower.L')) and newden['upper.L'][~explicit_left].min()>.01),
...
basic_checks.update(extra_checks)
...
report = dict(complete=True,passed_analytic=passed,passed_geometry=False,accepted_geometry=False,
              ...
              basic_checks=basic_checks,errors=errors,curve_verification=curve_report,extra_arm_transports=extra_report,
              ...
              unused_new_hand_radial_projection_note='Changed poses use explicitly exported transports for all four arm bones. Their new hand radial and candidate absolute rest-map denominators are diagnostic only because they do not define the applied changed frames. Original/fallback guards retain their old limits.',
