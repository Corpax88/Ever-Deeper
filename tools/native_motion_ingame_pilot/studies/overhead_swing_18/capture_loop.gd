extends SceneTree
## Actual held mining in the unchanged world; loop visibility only, no release gate.
var output := ""
var frames := ""
var main: Node
var world: Node
...
func _run() -> void:
	...
	main.get_node("EndlessDescentWorld/Player/Visual").set_script(load("res://tools/native_motion_ingame_pilot/studies/overhead_swing_18/loop_visual.gd"))
	root.add_child(main)
	current_scene = main
	...
	file.close()
	print("OVERHEAD_SWING_GAME_COMPLETE ",hits," ",damage)
