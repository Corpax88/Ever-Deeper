...
class SimpleSwing:
    ...
    def __init__(self,anchors):
        ...
        contact_axis=Vector((-.15,-.98,.13)).normalized()
        contact_rotation=self.frame(contact_axis)
        ...
        self.keys=[
            ...
            (.34,Vector((.05,.44,1.64)),Vector((-.10,-.40,1.)),5.,.20),
            (HIT,contact_rear,contact_axis,-12.,1.),
            (.52,contact_rear,contact_axis,-12.,1.),
        ...
        for _,_,_,lean,weight in self.keys:
            ...
            body={k:bend@m for k,m in body.items()}
            # Keep the gaze on the rock while the chest folds. A neck-local
            # counterrotation prevents the helmet from following the chest
            # down across the fixed pick tip. It does not detach the head.
            neck=body['head']@Vector((0.,0.,1.25))
            look=Matrix.Translation(neck)@Matrix.Rotation(math.radians(18.*weight),4,'Y')@Matrix.Translation(-neck)
            body['head']=look@body['head']
