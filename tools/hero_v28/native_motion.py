...
def sample(gear, state, phase, ground_per_pixel, speed=340.):
    ...
    return pm.orient_pose(p, ground_per_pixel)
...
class Transition:
    ...
    def __post_init__(self):
        ...
        self.source = sample(self.gear, self.source_state, self.source_phase, self.ground, self.speed)
        ...
        if self.source_state == 'walk' and self.target_state != 'walk':
            ...
            if self.support_side is not None:
                ...
                eps = 1e-4
                before = sample(self.gear, 'walk', self.source_phase-eps/self.source_duration, self.ground, self.speed)
                after = sample(self.gear, 'walk', self.source_phase+eps/self.source_duration, self.ground, self.speed)
                ...
                target = sample(self.gear, self.target_state, self.target_phase, self.ground, self.speed)
    ...
    def sample(self, seconds):
        ...
        source = pm.translate_pose(sample(self.gear, self.source_state, self.phase_at(self.source_state, self.source_phase, t),
                                         self.ground, self.speed), source_root)
        target = pm.translate_pose(sample(self.gear, self.target_state, self.phase_at(self.target_state, self.target_phase, t),
                                         self.ground, self.speed), target_root+self.destination_offset)
