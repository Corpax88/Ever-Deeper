...
function injectSuiteArgument(html) {
  ...
  config.args = [...suiteArgs, SUITE_ARG, GAMEPLAY ? "--overhaul-gameplay" : "--overhaul-capture", ...(process.env.MENU_TOUCH === "1" ? ["--menu-touch-only"] : []), "--pet-capture", ...(process.env.PET_REACTIONS === "1" ? ["--pet-reactions"] : []), `--capture-start=${RANGE_START}`, `--capture-end=${RANGE_END}`];
    ...
    if (GAMEPLAY) {
      ...
      while (true) {
        ...
        if (input.kind === "record_start") {
          ...
          await page.evaluate(() => { const canvas = document.getElementById("canvas"); for (const type of ["keydown", "keyup"]) canvas.dispatchEvent(new KeyboardEvent(type, {key:"F13", code:"F13", keyCode:124, which:124, bubbles:true, cancelable:true})); }); continue;
        }
        ...
        if (input.kind === "capture") {
          ...
          await page.evaluate(() => { const canvas = document.getElementById("canvas"); for (const type of ["keydown", "keyup"]) canvas.dispatchEvent(new KeyboardEvent(type, {key:"F13", code:"F13", keyCode:124, which:124, bubbles:true, cancelable:true})); });
          continue;
        }
        gestureIndex++;
        if (gestureIndex===1) {
        ...
        if (input.kind === "tap") await page.touchscreen.tap(x,y);
          ...
          if (input.kind === "swipe" || input.kind === "hold") {
            for(let i=1;i<=8;i++) {
              await cdp.send("Input.dispatchTouchEvent",{type:"touchMove",touchPoints:[{x:x+input.dx/input.width*box.width*i/8,y:y+input.dy/input.height*box.height*i/8}]});
