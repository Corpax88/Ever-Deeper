# Ordinary startup comparison — preparation only

Four disposable Mac WebKit launches on one origin/path: published DEV13 and
the nonindexed warmup candidate, each with fresh storage and with the same
unchanged normal v3 Surface save. All nine exported files remain literal.
There is no QA argument, engine/GL patch, runtime handler call or seed override.

The saved fixture is the original `save-surface.sav` from shader identity
artifact 10502243075 (run 35232918221). Only that primary file is populated;
backup, audio settings, tutorial configuration and all other user files are
absent in both prepared contexts. This deliberately lets the normal tutorial
run. Every IndexedDB row is recorded before launch and before/after actions.

A passive DOM observer records splash/status lifecycle and trusted events.
After the original loading overlay is removed, the fresh pair gets a short
ArrowRight press/release; the saved pair holds ArrowRight until the settled
menu is captured, then releases. This is **early visible-launch input**.
It cannot establish overlap with Main/helper initialization or prove when
Godot installed its event listeners; that boundary remains inconclusive.

OCR identifies the settled menu and the actual NEW GAME/CONTINUE control.
The natural tutorial is retained. At first settled HUD, the Surface save must
still have its expected starting location. A normal 500 ms ArrowRight hold
and release follows. A naturally persisted bounded position change is then
observed, followed by 16 seconds of uninterrupted gameplay/video/frames and
unchanged saved location. The source has an 8 s location checkpoint and 6 s
autosave queue; unchanged bytes alone are not a new-checkpoint claim.
The predeclared displacement envelope is 100–230 logical pixels rightward,
within the clear camp terrace. No mining input or mining-release claim is made.

Full-resolution PNGs, a continuous video, raw errors, lifecycle, actions,
storage and exact package hashes are retained. Missing splash or uncertain
early engine-input coverage is inconclusive, not silently accepted.
Visual/state parity requires independent review of the actual four outputs;
a zero runner exit only means its bounded mechanical checks passed.

No REQUEST exists in a preparation commit. A separately reviewed direct child
may add only REQUEST.json for four sessions, attempt 1. There is no rerun,
publication, FPS or physical-iPhone claim.
