...
function injectSuiteArgument(html) {
  ...
  config.args = [...suiteArgs, SUITE_ARG, GAMEPLAY ? "--overhaul-gameplay" : (process.env.HERO_V28_CAPTURE === "1" ? "--hero-capture" : "--overhaul-capture"), ...(process.env.HERO_V28_CAPTURE === "1" ? ["--hero-v28-capture"] : []), ...(process.env.CAPTURE_SCOPE === "light" ? ["--light-capture"] : []), ...(process.env.MENU_TOUCH === "1" ? ["--menu-touch-only"] : []),  ...(process.env.PET_REACTIONS === "1" ? ["--pet-reactions"] : []), `--capture-start=${RANGE_START}`, `--capture-end=${RANGE_END}`];
...
async function captureSuite(options) {
  ...
  try {
    ...
    if ((typeof versionBadge.version !== "string" || versionBadge.version.length === 0 || (process.env.EXPECTED_VERSION && versionBadge.version !== process.env.EXPECTED_VERSION)) || versionBadge.badge !== "v" + versionBadge.version) throw new Error("Cached loading version badge did not synchronize: " + JSON.stringify(versionBadge));
