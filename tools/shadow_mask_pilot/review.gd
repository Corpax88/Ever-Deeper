...
func _pair(label: String) -> void:
	original.refresh()
	candidate.refresh()
	var row: Dictionary = {"id":label,"original_count":original.active_count,"candidate_count":candidate.active_count,
		"original_polygons":_polygons(original),"candidate_polygons":_polygons(candidate)}
	if row.original_polygons != row.candidate_polygons:
		failure = "Original rectangle geometry changed"; pairs.append(row); return
	...
	for mode in [0, 1, 0]:
		...
		candidate.refresh()
		if mode == 1:
			row.eligible_pairs = candidate.eligible_pairs
			row.baseline_pairs = candidate.baseline_pairs
			row.fallback = candidate.fallback_reason
			row.mask_update_usec = candidate.mask_update_usec
		for frame in 3: await process_frame
	...
	print("MASK_PAIR " + JSON.stringify({"id":label,"candidate":row.candidate,"restored":row.restored,"failure":failure}))
...
func _finish() -> void:
	FileAccess.open(output.path_join("mask-review.json"),FileAccess.WRITE).store_string(JSON.stringify({
		"physical_iphone":false,"adopted":false,"baseline":"1b9cfe26485c0f01dade7f2bbd6153cbc3c5fb9a","renderer":RenderingServer.get_video_adapter_name(),
	...
	print("SHADOW_MASK_REVIEW_COMPLETE " + JSON.stringify({"pairs":pairs.size(),"failure":failure}))
