...
const BRANCH='refs/heads/codex/dev11-webkit-hitch-probe-20260917';
const IDENTITY=process.env.DIAGNOSTIC_IDENTITY_SHA256;
...
const result={schema:1,source:SOURCE,diagnostic_only:true,pck_sha256:null,metric:'engine_loop_callback_cadence',rendered_fps_verified:false,physical_iphone:false,started:stamp(),complete:false};
...
async function phaseClocks(label) {
  const pairs=await page.evaluate(()=>Array.from({length:5},()=>{
    const js_before=performance.now(),receipt=window.__everDeeperHitchProbe('clock'),js_after=performance.now();
    return {js_before,receipt,js_after};
  }));
  await write('clock-'+label+'.json',pairs);return pairs;
}
try {
  ...
  if(request.sessions!==1||request.source!==SOURCE||request.diagnostic_only!==true||!request.preparation_sha)throw Error('Missing one-session authorization');
  ...
  if(identity.diagnostic_only!==true||identity.productionBase!==SOURCE||identity.sourceSha!==process.env.GITHUB_SHA||identity.preparationSha!==request.preparation_sha||Object.keys(identity.devFiles).length!==9)throw Error('Wrong nine-file source');
  const verified={};
  ...
  result.pck_sha256=verified['index.pck'].sha256;result.diagnostic_source=identity.sourceSha;
  ...
  const clockBefore=await phaseClocks('before');
  const probeStart=await page.evaluate(()=>window.__everDeeperHitchProbe('start'));
  await write('phase-start.json',probeStart);
  if(!probeStart.started)throw Error('Diagnostic recorder did not start');
  ...
  const probeStop=await page.evaluate(()=>window.__everDeeperHitchProbe('stop'));
  await write('phase-stop.json',probeStop);
  if(!probeStop.stopped||probeStop.open_parent!==-1)throw Error('Diagnostic recorder did not close');
  const clockAfter=await phaseClocks('after');
  const phaseCapture=await page.evaluate(()=>window.__everDeeperHitchProbe('export'));
  await write('phase-capture.json',phaseCapture);
  await write('raw-loop.json',raw);
  result.phases=analyzePhases(phaseCapture,{before:clockBefore,after:clockAfter},raw);
  await write('phase-analysis.json',result.phases);
  result.phases={events:phaseCapture.count,dropped:phaseCapture.dropped,integrity_errors:phaseCapture.integrity_errors,clock:result.phases.clock,kinds:result.phases.kinds,all_phase_union_ms:result.phases.all_phase_union_ms,overhead_bearing:true};
  ...
  result.cpu_sampling={status:'not_part_of_bounded_phase_probe',exclusive_gpu_time_available:false};
  ...
  if(page) {
    ...
    await bounded(page.evaluate(()=>{if(!window.__everDeeperHitchProbe)return null;const stop=window.__everDeeperHitchProbe('stop');return {stop,capture:window.__everDeeperHitchProbe('export')};}).then(x=>write('failure-phases.json',x)),3000,'failure phase receipt').catch(()=>{});
  ...
  console.log((result.complete?'WEBKIT_HITCH_PROBE_COMPLETE':'WEBKIT_HITCH_PROBE_FAILED')+' '+JSON.stringify({source:SOURCE,pck:result.pck_sha256,complete:result.complete,metric:result.metric,error:result.error||null}));
}
