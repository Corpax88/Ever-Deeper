...
def verify(candidate=None):
    ...
    if candidate:
        ...
        assert identity['sourceSha'] == pins['source'] and identity['runId'] == pins['candidate_run']
        ...
        for name, expected in pins['files'].items():
            ...
            files[name] = expected
        shipped = (candidate / 'index.js').read_text()
        ...
        for method in binding['absent_2d_subimage_names']:
    ...
    return {'schema': 1, 'production_source': pins['source'], 'runtime_tree_sha256': SHA(tree),
            ...
            'no_build_or_runtime_mutation': True}
