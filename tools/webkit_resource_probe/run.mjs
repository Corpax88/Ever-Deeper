...
const BRANCH='refs/heads/codex/dev11-webkit-resource-probe-20260917';
...
try {
  ...
  if(request.sessions!==1||request.source!==SOURCE||request.diagnostic_only!==true||request.probe!=='selected_webgl_resources_v1'||!request.preparation_sha)throw Error('Missing one-session authorization');
  ...
  if(identity.sourceSha!==SOURCE||identity.runId!=='35186932201'||Object.keys(identity.devFiles).length!==9)throw Error('Wrong nine-file source');
  const verified={};
  ...
  if(result.pck_sha256!=='5b77b3a219011cb676895e41830c8dab32bec2346db93102c7894a3c084414e9')throw Error('Wrong original DEV11 PCK');
  ...
  if(!result.resource_restoration.all_methods_restored||!result.resource_restoration.descriptors_restored||
     !result.resource_restoration.get_context_restored_to_observer||result.resource_restoration.method_count!==7||result.resource_restoration.error_mask)throw Error('Resource method restoration failed');
  ...
  console.log((result.complete?'WEBKIT_RESOURCE_PROBE_COMPLETE':'WEBKIT_RESOURCE_PROBE_FAILED')+' '+JSON.stringify({source:SOURCE,pck:result.pck_sha256,complete:result.complete,metric:result.metric,error:result.error||null}));
}
