...
func _process(delta: float) -> void :
	...
	if stamp != 0: QAProfile.record("minimap_overlay._process", Time.get_ticks_usec() - stamp)
...
func _draw() -> void :
	var stamp: int = Time.get_ticks_usec() if QAProfile.enabled else 0
	_qa_impl_draw()
	if stamp != 0: QAProfile.record("minimap_overlay._draw", Time.get_ticks_usec() - stamp)
