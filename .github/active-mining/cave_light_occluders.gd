...
func _process(_delta: float) -> void:
	...
	_qa_impl_process(_delta)
	if stamp != 0: QAProfile.record("cave_light_occluders._process", Time.get_ticks_usec() - stamp)

func _qa_impl_process(_delta: float) -> void:
	refresh()

func refresh() -> void:
	var stamp: int = Time.get_ticks_usec() if QAProfile.enabled else 0
	_qa_implrefresh()
	if stamp != 0: QAProfile.record("cave_light_occluders.refresh", Time.get_ticks_usec() - stamp)

func _qa_implrefresh() -> void:
