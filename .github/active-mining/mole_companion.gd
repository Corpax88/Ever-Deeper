...
func _physics_process(delta: float) -> void:
	...
	_qa_impl_physics_process(delta)
	if stamp != 0: QAProfile.record("mole_companion._physics_process", Time.get_ticks_usec() - stamp)

func _qa_impl_physics_process(delta: float) -> void:
...
func _path_to(point: Vector2) -> Array[Vector2]:
	var stamp: int = Time.get_ticks_usec() if QAProfile.enabled else 0
	var result = _qa_impl_path_to(point)
	if stamp != 0: QAProfile.record("mole_companion._path_to", Time.get_ticks_usec() - stamp)
	return result

func _qa_impl_path_to(point: Vector2) -> Array[Vector2]:
	path_searches+=1
	var tile: float = 64.0 if world.has_method("_is_floor") else 48.0
