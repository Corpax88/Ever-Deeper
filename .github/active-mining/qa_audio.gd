...
func _process(delta: float) -> void :
	...
	_qa_impl_process(delta)
	if stamp != 0: QAProfile.record("audio._process", Time.get_ticks_usec() - stamp)

func _qa_impl_process(delta: float) -> void :
...
func _prepare_music_samples() -> void:
	# Keep one private, non-looping stream per track. Decode web samples while
	# the game is loading, never on the frame that begins a music crossfade.
	for original in MUSIC_TRACKS:
		var stream: AudioStream = original.duplicate()
		if stream is AudioStreamMP3:
			stream.loop = false
		_prepared_music.append(stream)
...
func _start_music_track(slot: int, track_index: int, fade_in: bool) -> void :
	var stamp: int = Time.get_ticks_usec() if QAProfile.enabled else 0
	_qa_impl_start_music_track(slot, track_index, fade_in)
	if stamp != 0: QAProfile.record("audio._start_music_track", Time.get_ticks_usec() - stamp)

func _qa_impl_start_music_track(slot: int, track_index: int, fade_in: bool) -> void :
	...
	if QAProfile.enabled: QAProfile.record("audio.assign", Time.get_ticks_usec()-mark)
	mark = Time.get_ticks_usec()
	player.play()
	if QAProfile.enabled: QAProfile.record("audio.play", Time.get_ticks_usec()-mark)
