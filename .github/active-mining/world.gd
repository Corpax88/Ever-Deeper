...
var qa_stable_strips: bool = false
...
func _mine_once() -> void :
	var stamp: int = Time.get_ticks_usec() if QAProfile.enabled else 0
	_qa_impl_mine_once()
	if stamp != 0: QAProfile.record("world._mine_once", Time.get_ticks_usec() - stamp)

func _qa_impl_mine_once() -> void :
...
func _find_mine_target() -> Vector2i:
	var stamp: int = Time.get_ticks_usec() if QAProfile.enabled else 0
	var result = _qa_impl_find_mine_target()
	if stamp != 0: QAProfile.record("world._find_mine_target", Time.get_ticks_usec() - stamp)
	return result

func _qa_impl_find_mine_target() -> Vector2i:
	if not active:
		return Vector2i(-1, -1)
...
func _update_drops(delta: float) -> void :
	var stamp: int = Time.get_ticks_usec() if QAProfile.enabled else 0
	_qa_impl_update_drops(delta)
	if stamp != 0: QAProfile.record("world._update_drops", Time.get_ticks_usec() - stamp)

func _qa_impl_update_drops(delta: float) -> void :
...
func _draw() -> void :
	var stamp: int = Time.get_ticks_usec() if QAProfile.enabled else 0
	_qa_impl_draw()
	if stamp != 0: QAProfile.record("world._draw", Time.get_ticks_usec() - stamp)

func _qa_impl_draw() -> void :
	_draw_canvas = self
	if mine.is_empty():
		return
...
func _exposed_floor_regions(start: Vector2i, finish: Vector2i) -> Array[Rect2]:
	var stamp: int = Time.get_ticks_usec() if QAProfile.enabled else 0
	var result = _qa_impl_exposed_floor_regions(start, finish)
	if stamp != 0: QAProfile.record("world._exposed_floor_regions", Time.get_ticks_usec() - stamp)
	return result

func _qa_impl_exposed_floor_regions(start: Vector2i, finish: Vector2i) -> Array[Rect2]:
...
func _draw_partitioned_mine(start: Vector2i, finish: Vector2i) -> void:
	var stamp: int = Time.get_ticks_usec() if QAProfile.enabled else 0
	_qa_impl_draw_partitioned_mine(start, finish)
	if stamp != 0: QAProfile.record("world._draw_partitioned_mine", Time.get_ticks_usec() - stamp)

func _qa_impl_draw_partitioned_mine(start: Vector2i, finish: Vector2i) -> void:
	if qa_stable_strips:
		start.x = maxi(0, floori(float(start.x) / 6.0) * 6)
		finish.x = mini(cols - 1, ceili(float(finish.x + 1) / 6.0) * 6 - 1)
