...
func serialize() -> Dictionary:
	...
	var result = _qa_implserialize()
	if stamp != 0: QAProfile.record("state.serialize", Time.get_ticks_usec() - stamp)
	return result

func _qa_implserialize() -> Dictionary:
...
func save_game(path: String = "") -> bool:
	var stamp: int = Time.get_ticks_usec() if QAProfile.enabled else 0
	var result = _qa_implsave_game(path)
	if stamp != 0: QAProfile.record("state.save_game", Time.get_ticks_usec() - stamp)
	return result

func _qa_implsave_game(path: String = "") -> bool:
...
func _state_changed() -> void :
	var stamp: int = Time.get_ticks_usec() if QAProfile.enabled else 0
	_qa_impl_state_changed()
	if stamp != 0: QAProfile.record("state._state_changed", Time.get_ticks_usec() - stamp)

func _qa_impl_state_changed() -> void :
