...
subprocess.run([sys.executable,str(source/'apply_delta.py'),str(out/'index.pck'),str(source/'ground-input-delta'),str(out/'index.pck')],check=True)
subprocess.run([sys.executable,str(source/'apply_delta.py'),str(out/'index.pck'),str(source/'input-state-delta'),str(out/'index.pck')],check=True)
subprocess.run([sys.executable,str(source/'apply_delta.py'),str(out/'index.pck'),str(source/'final-gesture-delta'),str(out/'index.pck')],check=True)
...
print('Exact overhaul candidate verified',target['version'])