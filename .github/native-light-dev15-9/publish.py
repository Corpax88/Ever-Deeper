...
def reviewed():
 review=read(HERE/'review.json');manifest=read(HERE/'manifest.json');baseline=read(HERE/'baseline.json');worn=read(HERE.parent/'native-flow-trial/bundle.json')
 ...
 required={'manifest.json','baseline.json','native-input.json','evidence/study.json','evidence/light-webkit.json','evidence/core.json','evidence/build.json','evidence/webkit.json','evidence/performance.json','evidence/independent-review.json','publish.py','../workflows/publish-native-light-dev15-9.yml','../native-flow-trial/bundle.json'}
 ...
 require(baseline['displayed_dev_version']=='1.0.0-dev.15.8' and set(baseline['files'])=={'live','dev'},'Unexpected previous release')
 study=read(HERE/'evidence/study.json');build=read(HERE/'evidence/build.json');webkit=read(HERE/'evidence/webkit.json');critic=read(HERE/'evidence/independent-review.json');light=read(HERE/'evidence/light-webkit.json')
 for item in [study,build,webkit,critic,light]:require(item['files']==manifest and item['version']==VERSION and item['source_commit']==review['source_commit'],'Wrong package evidence')
 for item in [study,light]:require(item['passed'] and all(c['passed'] for c in item['checks']),'Browser check failed')
 require(len(study['pairs'])==22 and all(p['max_channel_difference']==0 and p['changed_channels']==0 and p['reference']==p['restored'] for p in study['pairs']),'Pixel parity failed')
 require(study['runtime']['canvas']==[2532,1170] and study['runtime']['dpr']==3 and 'Apple' in study['runtime']['renderer'],'Wrong measurement environment')
 ws=study['windows'];require([w['cached'] for w in ws]==[False,True,True,False,True,False],'Missing alternating timings')
 ref=[w for w in ws if not w['cached']];candidate=[w for w in ws if w['cached']]
 fps=lambda rows:sum(w['frames'] for w in rows)/sum(w['seconds'] for w in rows)
 require(fps(candidate)>fps(ref)*1.04 and min(w['frames']/w['seconds'] for w in candidate)>max(w['frames']/w['seconds'] for w in ref),'No consistent measured frame benefit')
 require(max(w['frame']['p95_ms'] for w in candidate)<min(w['frame']['p95_ms'] for w in ref),'p95 regression')
 require(all(w['draw_calls']==220 and w['impacts']>=5 and w['frames']>200 for w in ws),'Changed workload')
 names={'version','depth-one-started','completed','restored','canvas-unchanged','all-stage-markers','end-marker','no-network-during-test','pet-lights-isolated','shadows-isolated','all-lights-isolated','original-lights-return','phase-boundaries','result-fits','pending-report-protected','exact-labelled-transfer','receipt-clears-report','cancel-restores-1','cancel-stops-report-1','cancel-restores-3','cancel-stops-report-3','cancel-restores-5','cancel-stops-report-5','no-script-errors'}
 require(names<={c['name'] for c in light['checks']},'Missing report checks')
 core=read(HERE/'evidence/core.json');require(core['passed'] and all(c['passed'] for c in core['cases']) and {c['case'] for c in core['cases']}=={'input','touch','build-flavor'},'Core failed')
 require(build['native_files_unchanged']==read(HERE/'native-input.json') and build['save_path']=='user://ever_deeper_dev_run_v3.sav','Native assets or save path changed')
 ...
 require(critic['accepted'] and critic['final_build_images_inspected'] and min(critic['scores'].values())>=8,'Independent acceptance missing')
...
def prepare(work,candidate):
 ...
 require(origin['name']=='native-light-dev15-9-candidate' and origin['digest']==review['export_artifact']['digest'] and origin['workflow_run']['id']==review['export_run_id'],'Wrong original export artifact')
 ...
 require({label:want['name'] for label,want in review['artifacts'].items()}=={'candidate':'native-light-dev15-9-candidate','build':'native-light-dev15-9-build','browser':'native-light-dev15-9-browser'},'Required artifact set incomplete')
 ...
 require(review['artifacts']['candidate']['name']=='native-light-dev15-9-candidate','Wrong candidate artifact')
 ...
 print('NATIVE_LIGHT_DEV15_9_STAGED reviewed_files=9 preserved=18')
def verify(output):
 ...
 for attempt in range(10):
  ...
  if not pending:
   (output/'publication-receipt.json').write_text(json.dumps({'passed':True,'version':VERSION,'destination':PUBLIC+'dev/','source_commit':review['source_commit'],'run_id':review['artifact_run_id'],'files':expected,'live_files_unchanged':9,'retained_trial_files_unchanged':9,'physical_iphone_verified':False,'verification_scope':'Depth-one native shader:22 pixel-exact A/B/A pairs and six Mac Apple Metal mining windows54.47 to57.35FPS. WebKit report/startup and exported input/touch/save passed. GPU reduction inconclusive; physical iPhone improvement unverified.'},indent=2)+'\n')
   print('NATIVE_LIGHT_DEV15_9_PUBLIC_VERIFY_OK files=27');return
