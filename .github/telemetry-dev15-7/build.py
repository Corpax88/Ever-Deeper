...
def build(godot,work):
 ...
 expected=json.loads((ROOT/'.github/telemetry-dev15-7/shell-input.json').read_text())
 ...
 html=html.replace('const GODOT_CONFIG = ',(ROOT/'tools/session-reports-browser.js').read_text()+'\nconst GODOT_CONFIG = ',1)
 ...
 (work/'build.json').write_text(json.dumps({'version':'1.0.0-dev.15.7','source_commit':os.environ.get('GITHUB_SHA'),'files':files,'save_path':'user://ever_deeper_dev_run_v3.sav','native_carrier':identity(carrier),'native_files_unchanged':native,'physical_iphone_verified':False},indent=2)+'\n')
