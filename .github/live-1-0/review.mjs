...
const version='1.0.0',files=JSON.parse(fs.readFileSync(path.join(web,'manifest.json')));
...
const server=http.createServer((req,res)=>{
 ...
 if(name==='/index.html')res.end(fs.readFileSync(file,'utf8').replace(/const GODOT_CONFIG = (\{[^\r\n]+\});/,(_,raw)=>{const c=JSON.parse(raw);c.args=['--','--qa-production-browser','--expected-version='+version];return 'const GODOT_CONFIG = '+JSON.stringify(c)+';';}));
...
try{
 ...
 check('production-flavor',!initial.dev_feature&&!initial.dev_menu_present&&!initial.dev_menu_resource&&!initial.render_probe_resource&&initial.save_path==='user://ever_deeper_run_v3.sav'&&initial.user_dir_name==='Ever Deeper- Godot Production Port'&&!initial.release_label.includes('DEV'),{state:initial});
 await shot('production-start-menu');
 await tap('new_game');await ready();await shot('production-new-game');await command('surface');
 ...
 await command('checkpoint');await delay(1200);const saved=await state();
 check('production-save-written',saved.save_file_present&&saved.save_error===0,{state:saved});
 await page.reload({waitUntil:'domcontentloaded',timeout:180000});
 const restored=await wait('saved production menu',s=>s?.version===version&&s.save_available&&s.menu,240000);
 ...
 await shot('production-saved-menu');await tap('continue');await ready();await shot('production-continued-game');
