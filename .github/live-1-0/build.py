"""Build LIVE1.0 while preserving the exact approved native hero/tool resources."""
...
def build(godot,work):
 ...
 run([godot,'--headless','--path',str(ROOT),'--script',str(ROOT/'.github/live-1-0/extract-assets.gd'),'--',str(carrier),str(ROOT)],work/'extract.log')
 native=json.loads((ROOT/'.github/live-1-0/native-input.json').read_text())
 ...
 run([godot,'--headless','--path',str(ROOT),'--export-pack','Web Production',str(candidate/'index.pck')],work/'export.log')
 with concurrent.futures.ThreadPoolExecutor(max_workers=4) as pool:
  list(pool.map(lambda n:download(n,candidate/n),[n for n in NAMES if n!='index.pck']))
 expected=json.loads((ROOT/'.github/live-1-0/shell-input.json').read_text())
 for n,want in expected['files'].items(): assert identity(candidate/n)==want,'Web runtime changed: '+n
 ...
 assert hashlib.sha256(normalized.encode()).hexdigest()==expected['normalized_html_sha256'],'Web shell changed'
 ...
 (work/'build.json').write_text(json.dumps({'version':'1.0.0','source_commit':os.environ.get('GITHUB_SHA'),'files':files,'save_path':'user://ever_deeper_run_v3.sav','native_carrier':identity(carrier),'native_files_unchanged':native,'physical_iphone_verified':False},indent=2)+'\n')
 print('LIVE1_EXPORT_COMPLETE')
