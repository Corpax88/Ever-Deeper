"""Publish only the exact reviewed LIVE1.0 artifact; preserve DEV and Worn trial."""
...
def reviewed():
 ...
 require(review['destination']=='live' and review['version']==VERSION and review['live_authorized'] is True,'Wrong authorized destination')
 ...
 require({'manifest.json','baseline.json','native-input.json','evidence/browser.json','evidence/core.json','evidence/build.json','evidence/skills-core.log','evidence/webkit.json','evidence/independent-review.json','publish.py','../workflows/publish-live-1-0.yml','../native-flow-trial/bundle.json'}<=set(review['reviewed_files']),'Missing pinned evidence')
 require(baseline['displayed_dev_version']=='1.0.0-dev.15.5' and set(baseline['files'])=={'live','dev'},'Unexpected previous release')
 ...
 require({'production-flavor','production-save-written','production-save-reloaded','production-start-menu','production-new-game','production-saved-menu','production-continued-game','graphical-mac-renderer','larger-gameplay-controls-844','larger-gameplay-controls-667','larger-gameplay-controls-3-2','hud-top-and-context-clear-844','hud-top-and-context-clear-667','hud-top-and-context-clear-3-2','approved-icon-opens-skills-844','approved-icon-opens-skills-667','approved-icon-opens-skills-3-2','hud-844','hud-667','hud-3-2','hud-moss-844','touch-mining-after-menu','no-runtime-errors','relocated-quarry-and-retired-surface-store','relocated-quarry-mining','road-walkable-after-relocation','no-wayfarer-mossMine','no-wayfarer-moonMine','no-wayfarer-emberMine','no-wayfarer-starMine','quarry-full-844','quarry-full-667','quarry-stage-120','quarry-stage-0','hover-stat-mining','hover-stat-running','hover-stat-carrying','hover-stat-prospecting','hover-stat-stamina','tooltip-mining-844','tooltip-running-844','tooltip-carrying-844','tooltip-prospecting-844','tooltip-stamina-844','skill-bars-844','skill-bars-667','skill-bars-3-2','skill-bars-edges','zero-near-full-and-cap','xp-rollover','skills-edges-844','skills-rollover-844','long-press-stat-844'}<=names,'Missing affected HUD behavior or target captures')
 ...
 require(core['passed'] and all(c['passed'] for c in core['cases']) and {c['case'] for c in core['cases']}=={'input','overhaul','touch','layout','build-flavor','one-point-zero-migration'},'Core gameplay failed')
 ...
 require(build['save_path']=='user://ever_deeper_run_v3.sav','Production save namespace changed')
...
def prepare(work,candidate):
 ...
 require({label:want['name'] for label,want in review['artifacts'].items()}=={'candidate':'live-1-0-candidate','build':'live-1-0-build-review','browser':'live-1-0-browser-review'},'Required artifact set incomplete')
 ...
 require(review['artifacts']['candidate']['name']=='live-1-0-candidate','Wrong candidate artifact')
 ...
 for n in FILES:shutil.copy2(candidate/n,site/n)
 shutil.copytree(work/'previous/dev',site/'dev')
 ...
 for n,want in baseline['files']['dev'].items():require(identity(site/'dev'/n)==want,'DEV changed')
 ...
 (work/'staging.json').write_text(json.dumps({'version':VERSION,'source_commit':review['source_commit'],'preserved_files':18,'live_files':9,'site_bytes':size},indent=2))
 print('LIVE1_STAGED reviewed_files=9 preserved=18')
def verify(output):
 review,manifest,baseline,worn=reviewed();output.mkdir(parents=True,exist_ok=False)
 expected={**manifest,**{'dev/'+k:v for k,v in baseline['files']['dev'].items()},**{'dev/worn/'+k:v for k,v in worn['files'].items()}};pending=list(expected)
 def check(name):
 ...
 for attempt in range(10):
  ...
  if not pending:
   (output/'publication-receipt.json').write_text(json.dumps({'passed':True,'version':VERSION,'destination':PUBLIC,'source_commit':review['source_commit'],'run_id':review['artifact_run_id'],'files':expected,'live_files_updated':9,'dev_files_unchanged':9,'retained_trial_files_unchanged':9,'physical_iphone_verified':False,'verification_scope':'Production without developer resources, observed production save namespace and persisted progress across reload, and exact exported Mac Chromium gameplay HUD at 844x390, 667x375 and 900x600, retired surface and four depth Wayfarer stores, relocated mountain full/damaged/rubble states, actual mining and road walking, existing Skills and normal startup, and normal Apple WebKit startup; physical iPhone remains unverified'},indent=2)+'\n')
   print('LIVE1_PUBLIC_VERIFY_OK files=27');return
