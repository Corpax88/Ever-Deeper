...
async function parity(label,retain=false){
 ...
 for(const [i,shot] of shots.entries()){
  ...
  check('parity-'+label+'-'+i,max<=1,{max,changed});
 }
}
async function measure(scenario,order){
 for(const [index,w] of order.entries()){
  ...
  windows.push({scenario,index,width:w,...ended.result,impacts,gpu});
...
try{
 ...
 for(const w of [6,3,1,6]){await width(w);await delay(5000);}await delay(60000);
 const order=process.env.REPEAT==='2'?[1,3,6,6,1,3,3,6,1]:[6,3,1,1,6,3,3,1,6];
 await measure('stationary',order);
 ...
 await command('freeze');await parity('moved');check('no-script-errors',!messages.some(m=>/SCRIPT ERROR|Parse Error|PAGEERROR|^error: ERROR:/.test(m)));
}catch(e){failed=String(e);console.error(e);}finally{
 fs.writeFileSync(path.join(output,'report.json'),JSON.stringify({passed:!failed,error:failed,source_commit:process.env.GITHUB_SHA,baseline_source:'c63aabd3e3e5120579285ce6e8b0b59a75f2727a',version,files,browser:browserName,repeat:process.env.REPEAT,runtime,checks,windows,pairs,captures,physical_iphone_verified:false,scope:'Diagnostic terrain strips6/3/1; all authored draws, lighting and native resolution retained. Browser study, no production change.'},null,2));
