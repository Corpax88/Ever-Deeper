...
def build(godot,work):
 ...
 expected=json.loads((ROOT/'.github/lighttest-dev15-8/baseline.json').read_text())
 for n,want in expected.items():
  if n != 'index.pck': assert identity(candidate/n)==want,'Web runtime changed: '+n
 html=(candidate/'index.html').read_text()
 original=(ROOT/'.github/lighttest-dev15-8/previous-reports.js').read_text()
 assert html.count(original)==1,'Existing report shell missing or duplicated'
 html=html.replace(original,(ROOT/'tools/session-reports-browser.js').read_text(),1)
 ...
 (work/'build.json').write_text(json.dumps({'version':'1.0.0-dev.15.8','source_commit':os.environ.get('GITHUB_SHA'),'files':files,'save_path':'user://ever_deeper_dev_run_v3.sav','native_carrier':identity(carrier),'native_files_unchanged':native,'physical_iphone_verified':False},indent=2)+'\n')
