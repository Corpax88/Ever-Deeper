...
const report={source_commit:process.env.GITHUB_SHA,physical_iphone:false,scope:'Instrumented mining fixture in actual Simulator Safari app; synthetic Godot mining action, not a native touch acceptance test. Local HTTP bridge adds observation cost. No release.',files,checks:[],windows:[],captures:[],documents:[],errors:[]};
let latest=null,pending=null,seq=0,session,driver,device,driverLog='';
const pause=ms=>new Promise(r=>setTimeout(r,ms));
const bridge=`<script>(()=>{let busy=false;window.addEventListener('error',e=>fetch('/__error',{method:'POST',body:String(e.message)}).catch(()=>{}));setInterval(async()=>{if(busy)return;busy=true;try{let g=window.DEV14_STATE?document.querySelector('canvas')?.getContext('webgl2'):null,e=g?.getExtension('WEBGL_debug_renderer_info');await fetch('/__state',{method:'POST',body:JSON.stringify({state:window.DEV14_STATE||null,ua:navigator.userAgent,width:innerWidth,height:innerHeight,dpr:devicePixelRatio,renderer:g?.getParameter(e?e.UNMASKED_RENDERER_WEBGL:g.RENDERER),canvas:g?[g.canvas.width,g.canvas.height]:null})});const d=await(await fetch('/__command')).json();if(d)window.DEV14_COMMAND=JSON.stringify(d)}catch(e){}finally{busy=false}},200)})();</script>`;
const server=createServer((req,res)=>{
 ...
 if(name==='/__state'||name==='/__error'){let b='';req.on('data',p=>b+=p);req.on('end',()=>{try{if(name==='/__state')latest={...JSON.parse(b),at:Date.now()};else report.errors.push(b)}catch{}res.writeHead(204).end()});return;}
 ...
 if(extname(file)==='.html'){report.documents.push({url:req.url,at:Date.now()});let s=fs.readFileSync(file,'utf8');s=s.replace(/const GODOT_CONFIG = (\{[^\r\n]+\});/,(_,raw)=>{let c=JSON.parse(raw);c.args=['--','--qa-fps-review'];return 'const GODOT_CONFIG = '+JSON.stringify(c)+';'});res.end(s.replace('</body>',bridge+'</body>'));}else fs.createReadStream(file).pipe(res);
});
...
async function wait(label,predicate,timeout=90000){for(let end=Date.now()+timeout;Date.now()<end;){if(latest?.state?.error)throw Error(latest.state.error);if(predicate(latest))return latest;await pause(250)}throw Error(label+' timeout: '+JSON.stringify(latest));}
...
try{
 ...
 const created=await wd('POST','/session',{capabilities:{alwaysMatch:{platformName:'iOS','appium:automationName':'XCUITest','appium:bundleId':'com.apple.mobilesafari','appium:udid':device.udid,'appium:platformVersion':'26.2','appium:noReset':true,'appium:autoWebview':false,'appium:orientation':'LANDSCAPE','appium:waitForIdleTimeout':0,'appium:wdaLaunchTimeout':600000,'appium:usePreinstalledWDA':true,'appium:prebuiltWDAPath':process.env.WDA_APP}}},660000);session=created.sessionId;
 ...
 await wait('actual Safari fixture menu',v=>v?.state?.menu,180000);report.runtime={...latest,state:undefined};check('iphone-agent',/iPhone/.test(latest.ua));check('landscape',latest.width>latest.height);await shot('01-menu');
...
finally{report.latest=latest;fs.writeFileSync(out+'/report.json',JSON.stringify(report,null,2));fs.writeFileSync(out+'/driver-stdout.log',driverLog);if(session)await command('DELETE','').catch(()=>{});if(driver?.pid)try{process.kill(-driver.pid,'SIGTERM')}catch{}server.closeAllConnections();await new Promise(r=>server.close(r));}
