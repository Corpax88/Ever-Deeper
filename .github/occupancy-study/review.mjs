...
async function width(value){await command('lightmode',{mode:value===0?'baseline':'occupancy_keys'});await delay(650);check('variant-'+value,(await state()).light_cost.mode===(value===0?'baseline':'occupancy_keys'));}
...
try{
 ...
 fs.writeFileSync(path.join(output,'report.json'),JSON.stringify({passed:!failed,error:failed,source_commit:process.env.GITHUB_SHA,baseline_source:'c63aabd3e3e5120579285ce6e8b0b59a75f2727a',version,files,browser:browserName,repeat:process.env.REPEAT,runtime,checks,windows,pairs,captures,physical_iphone_verified:false,scope:'QA shadow occupancy hash; exact original rendering, all lights and native resolution retained; no release.'},null,2));
