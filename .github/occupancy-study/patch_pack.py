...
def main(source, work):
    expected = json.loads((ROOT/'.github/occupancy-study/baseline.json').read_text())
    ...
    occluder = "scripts/lighting/cave_light_occluders.gd"
    old_mapping = occluder+".remap"
    assert old_mapping in entries
    replacements[old_mapping] = b'[remap]\npath="res://scripts/lighting/cave_light_occluders_qa.gd"\n'
    replacements["scripts/lighting/cave_light_occluders_qa.gd"] = (ROOT/occluder).read_bytes()
    ...
    (work/'build.json').write_text(json.dumps({
        ...
        'original_resource_count':len(before),'unchanged_resource_count':len(before)-2,
        'replaced_remaps':[remap,old_mapping],'original_remap':old_remap,'added_source_scripts':[fixture,helper,'scripts/lighting/cave_light_occluders_qa.gd'],
        'all_original_resource_md5_verified':True,'physical_iphone_verified':False,
    },indent=2)+'\n')
    print('QA_PCK_PATCH_VERIFIED',len(before)-2,'unchanged resources')
