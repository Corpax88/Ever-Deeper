...
if(JSON.stringify(files)!==JSON.stringify(JSON.parse(fs.readFileSync('.github/zero-light-study/manifest.json'))))throw Error('Pinned15.9manifest mismatch');
...
await context.addInitScript({path:'.github/zero-light-study/zero-light.js'});
...
async function command(kind,extra={}){
 if(kind==='reference'||kind==='cached'){
  await page.evaluate(value=>window.__edZeroLight.setEnabled(value),kind==='cached');
  kind='cached';
 }
 if(kind==='setup')extra={...extra,cached:true};
 const wanted=++id;await page.evaluate(d=>window.DEV14_COMMAND=JSON.stringify(d),{kind,id:wanted,...extra});return wait(kind,s=>s?.id===wanted);
}
...
try{
 ...
 const patch=await page.evaluate(()=>window.__edZeroLight.snapshot());check('shader-patch-active',patch.patched>0&&patch.activePrograms>0,{patch});
 fs.writeFileSync(path.join(output,'patched-shader.glsl'),(await page.evaluate(()=>window.__edZeroLight.sources()))[0]);
 ...
 fs.writeFileSync(path.join(output,'report.json'),JSON.stringify({passed:!failed,error:failed,source_commit:'c63aabd3e3e5120579285ce6e8b0b59a75f2727a',experiment_commit:process.env.GITHUB_SHA,version,files,runtime,checks,windows,pairs,physical_iphone_verified:false,scope:'Pinned public DEV15.9 runtime; only exact-zero-alpha positional-light early-out toggled by uniform. Custom light code excluded. Same scene A/B/A images and held mining; no physical phone claim.'},null,2));
