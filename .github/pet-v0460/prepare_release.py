"""Publish only the exact visually reviewed overhaul candidate and preserve LIVE."""
import sys,json,hashlib,urllib.request,shutil,time
from pathlib import Path
source=Path(__file__).parent
base={'live':json.loads((source/'live-baseline.json').read_text())['files'],'dev':json.loads((source/'dev-baseline.json').read_text())['files']}
target=json.loads((source/'candidate.json').read_text())
review=json.loads((source/'visual-review.json').read_text())
assert review['passed'] is True
assert review['pck_sha256']==target['files']['index.pck']['sha256']
assert review['viewport']==[932,430] and review['capture_count']==12
assert review['gameplay_checks']==796 and review['gameplay_passed'] is True
assert review['html_sha256']==target['files']['index.html']['sha256']
...
if '--verify-public' in sys.argv:
 ...
 print('All 18 public files match: original LIVE preserved, reviewed companion DEV published.')
else:
 ...
 print('Both previous versions verified; reviewed companion and unchanged LIVE packaged atomically.')
