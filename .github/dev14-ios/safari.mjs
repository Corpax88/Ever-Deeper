...
async function wd(method,path,body,timeout=840000){const r=await fetch('http://127.0.0.1:4723'+path,{method,headers:{'Content-Type':'application/json'},body:body===undefined?undefined:JSON.stringify(body),signal:AbortSignal.timeout(timeout)});const d=await r.json();if(!r.ok||d.value?.error)throw Error(JSON.stringify(d));return d.value;}
const command=(method,path,body)=>wd(method,'/session/'+session+path,body,90000);
...
try{
 ...
 const created=await wd('POST','/session',{capabilities:{alwaysMatch:{platformName:'iOS','appium:automationName':'XCUITest','appium:bundleId':'com.apple.mobilesafari','appium:udid':device.udid,'appium:platformVersion':'26.2','appium:noReset':true,'appium:autoWebview':false,'appium:orientation':'LANDSCAPE','appium:waitForIdleTimeout':0,'appium:wdaLaunchTimeout':600000,'appium:usePreinstalledWDA':true,'appium:prebuiltWDAPath':process.env.WDA_APP,'appium:showXcodeLog':false}}});session=created.sessionId;report.context=await command('GET','/context');assert.equal(report.context,'NATIVE_APP');
