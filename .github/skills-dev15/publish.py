"""Publish only the exact reviewed DEV15 artifact; preserve LIVE and Worn trial."""
...
def reviewed():
 ...
 require({'manifest.json','baseline.json','native-input.json','evidence/browser.json','evidence/core.json','evidence/build.json','evidence/skills-core.log','evidence/webkit.json','evidence/independent-review.json','publish.py','../workflows/publish-skills-dev15.yml','../native-flow-trial/bundle.json'}<=set(review['reviewed_files']),'Missing pinned evidence')
 require(baseline['displayed_dev_version']=='1.0.0-dev.14.3' and set(baseline['files'])=={'live','dev'},'Unexpected previous release')
 ...
 require({'graphical-mac-renderer','actual-joystick-trains-and-drains','active-rest-recovers','menu-freezes-stamina-and-training','embedded-map-survives-resize','small-phone-close-target','skills-hides-developer-overlay','closing-restores-developer-controls','fatigue-is-mild','open-during-mining-pauses-damage','resume-restores-movement','no-runtime-errors','02-surface-touch','10-exhausted-moss-touch','11-endless-touch','04-approved-skills-844','07-approved-skills-667','08-inventory','09-settings','09-settings-back','12-resumed-game'}<=names,'Missing affected behavior or target captures')
 ...
 require(core['passed'] and all(c['passed'] for c in core['cases']) and {c['case'] for c in core['cases']}=={'input','overhaul','touch','build-flavor'},'Core gameplay failed')
...
def prepare(work,candidate):
 ...
 require({label:want['name'] for label,want in review['artifacts'].items()}=={'candidate':'dev15-candidate','build':'dev15-build-review','browser':'dev15-browser-review'},'Required artifact set incomplete')
 ...
 require(review['artifacts']['candidate']['name']=='dev15-candidate','Wrong candidate artifact')
 ...
 print('SKILLS_DEV15_STAGED reviewed_files=9 preserved=18')
def verify(output):
 ...
 for attempt in range(10):
  ...
  if not pending:
   (output/'publication-receipt.json').write_text(json.dumps({'passed':True,'version':VERSION,'destination':PUBLIC+'dev/','source_commit':review['source_commit'],'run_id':review['artifact_run_id'],'files':expected,'live_files_unchanged':9,'retained_trial_files_unchanged':9,'physical_iphone_verified':False,'verification_scope':'Exact exported Mac Chromium mobile touch/gameplay/Skills capture and normal Apple WebKit startup; physical iPhone remains unverified'},indent=2)+'\n')
   print('SKILLS_DEV15_PUBLIC_VERIFY_OK files=27');return
