...
def main(source, work):
    expected = json.loads((ROOT/'.github/light-cost-study/baseline.json').read_text())
    ...
    fixture = 'scripts/qa/suites/fps_review_light_cost.gd'
    ...
    assert newbase == base
    assert all(after[n] == v for n,v in before.items() if n != remap)
    ...
    (work/'build.json').write_text(json.dumps({
        ...
        'original_resource_count':len(before),'unchanged_resource_count':len(before)-1,
        'replaced_remap':remap,'original_remap':old_remap,'added_source_scripts':[fixture,helper],
        'all_original_resource_md5_verified':True,'physical_iphone_verified':False,
    },indent=2)+'\n')
    print('QA_PCK_PATCH_VERIFIED',len(before)-1,'unchanged resources')
