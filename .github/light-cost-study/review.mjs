...
const modes=['baseline','frozen_occluders','hard_shadows','no_shadows','cones_off','bounce_off','lights_off'];
...
try{
 ...
 for(const value of modes){
  ...
  if(['baseline','frozen_occluders'].includes(value))identical(original,png,value+'-unchanged-image');
  if(['hard_shadows','no_shadows','lights_off'].includes(value)){fs.writeFileSync(path.join(output,value+'.png'),png);captures.push({name:value,state:s});}
 }
 await mode('baseline');identical(original,await page.screenshot(),'restored-original');
 await command('unfreeze');for(const value of modes){await mode(value);await delay(2500);}await mode('baseline');await delay(30000);
 ...
 const order=[...modes,...modes.slice(3),...modes.slice(0,3),...modes.slice(5),...modes.slice(0,5)];
 if(process.env.REPEAT==='2')for(let i=0;i<order.length;i+=7)order.splice(i,7,...order.slice(i,i+7).reverse());
 for(const [index,value]of order.entries()){
  ...
  const cost=ended.result.light_cost;check('refresh-ownership-'+index,value==='frozen_occluders'?cost.refresh_calls===0:cost.refresh_calls>150);
 ...
 fs.writeFileSync(path.join(output,'report.json'),JSON.stringify({passed:!failed,error:failed,source_commit:process.env.GITHUB_SHA,version,files,repeat:process.env.REPEAT,runtime,checks,windows,parity,captures,physical_iphone_verified:false,scope:'QA-only lighting ablations; full resolution; avatar and companion unchanged. Frozen occlusion updates are diagnostic only, not safe gameplay optimization. No release.'},null,2));
