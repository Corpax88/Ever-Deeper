...
def reviewed():
 ...
 require({'manifest.json','baseline.json','native-input.json','evidence/browser.json','evidence/core.json','evidence/build.json','evidence/skills-core.log','evidence/webkit.json','evidence/independent-review.json','publish.py','../workflows/publish-fps-dev15-6.yml','../native-flow-trial/bundle.json'}<=set(review['reviewed_files']),'Missing pinned evidence')
 ...
 require({'apple-gpu','version','terrain-cpu-improves','cache-reused','held-mining-reference-a','held-mining-cached','held-mining-reference-b','movement-after-mining','pixel-parity-ember-after-camera-move'}|{'pixel-parity-'+mine+'-'+state for mine in ['emberMine','mossMine','moonMine','starMine'] for state in ['intact','damaged','broken','restored']}<=names,'Missing targeted FPS/gameplay/parity checks')
 runtime=proof['runtime']
 require('Macintosh' in runtime['user_agent'] and runtime['dpr']==3 and ('Metal' in runtime['renderer'] or 'Apple' in runtime['renderer']),'Wrong graphical target')
...
def prepare(work,candidate):
 ...
 require({label:want['name'] for label,want in review['artifacts'].items()}=={'candidate':'dev15-6-candidate','build':'dev15-6-build-review','browser':'dev15-6-browser-review'},'Required artifact set incomplete')
 ...
 require(review['artifacts']['candidate']['name']=='dev15-6-candidate','Wrong candidate artifact')
 ...
 print('FPS_DEV15_6_STAGED reviewed_files=9 preserved=18')
def verify(output):
 ...
 for attempt in range(10):
  ...
  if not pending:
   (output/'publication-receipt.json').write_text(json.dumps({'passed':True,'version':VERSION,'destination':PUBLIC+'dev/','source_commit':review['source_commit'],'run_id':review['artifact_run_id'],'files':expected,'live_files_unchanged':9,'retained_trial_files_unchanged':9,'physical_iphone_verified':False,'verification_scope':'Four depth-one biome frozen terrain parity, Mac Metal A/B/A held-mining section setup and draw CPU, actual input and ordinary Apple WebKit startup; physical iPhone FPS and the recorded 250ms stall remain unverified'},indent=2)+'\n')
   print('FPS_DEV15_6_PUBLIC_VERIFY_OK files=27');return
