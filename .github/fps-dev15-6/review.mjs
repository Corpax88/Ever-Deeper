...
const version='1.0.0-dev.15.6',files=JSON.parse(fs.readFileSync(path.join(web,'manifest.json')));
...
async function parity(label){
 ...
 check('pixel-parity-'+label,reference===candidate,{reference,candidate});
}
try{
 ...
 check('terrain-cpu-improves',perSecond(candidate)<reference*.8,{reference_usec_per_second:reference,cached_usec_per_second:perSecond(candidate),reduction:1-perSecond(candidate)/reference});
 check('cache-reused',candidate.sections.reuses>candidate.sections.redraws*5,{sections:candidate.sections});
