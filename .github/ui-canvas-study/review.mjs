...
function valid(s,value){
 const c=s.light_cost;return c.mode===(value===0?'baseline':'no_empty_2d')&&c.empty_2d_disabled===(value===1)&&c.ui_combined===(value===1)&&c.ui_canvas_detached===(value===1)&&c.ui_child_count===3&&c.ui_visible&&c.parking_update_mode===(value===1?0:-1)&&c.native_viewports.length===1&&c.native_viewports.every(v=>v.canvas_items===0)&&c.lights.length===6&&c.lights.every(l=>l.enabled);
}
