...
def main(source, work):
    expected = json.loads((ROOT/'.github/fixed-merge-study/baseline.json').read_text())
    ...
    merger = "scripts/qa/fixed_light_merge.gd"
    assert merger not in entries
    replacements[merger] = (ROOT/merger).read_bytes()
    added_scripts.append(merger)
