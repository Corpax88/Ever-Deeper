...
def reviewed():
 ...
 require({'manifest.json','baseline.json','native-input.json','evidence/browser.json','evidence/core.json','evidence/build.json','evidence/skills-core.log','evidence/webkit.json','evidence/independent-review.json','publish.py','../workflows/publish-hud-dev15-2.yml','../native-flow-trial/bundle.json'}<=set(review['reviewed_files']),'Missing pinned evidence')
 require(baseline['displayed_dev_version']=='1.0.0-dev.15.1' and set(baseline['files'])=={'live','dev'},'Unexpected previous release')
 ...
 require({'graphical-mac-renderer','larger-gameplay-controls-844','larger-gameplay-controls-667','larger-gameplay-controls-3-2','hud-top-and-context-clear-844','hud-top-and-context-clear-667','hud-top-and-context-clear-3-2','approved-icon-opens-skills-844','approved-icon-opens-skills-667','approved-icon-opens-skills-3-2','hud-844','hud-667','hud-3-2','hud-moss-844','touch-mining-after-menu','no-runtime-errors'}<=names,'Missing affected HUD behavior or target captures')
...
def prepare(work,candidate):
 ...
 require({label:want['name'] for label,want in review['artifacts'].items()}=={'candidate':'dev15-2-candidate','build':'dev15-2-build-review','browser':'dev15-2-browser-review'},'Required artifact set incomplete')
 ...
 require(review['artifacts']['candidate']['name']=='dev15-2-candidate','Wrong candidate artifact')
 ...
 print('HUD_DEV15_2_STAGED reviewed_files=9 preserved=18')
def verify(output):
