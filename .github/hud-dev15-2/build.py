...
def build(godot,work):
 ...
 run([godot,'--headless','--path',str(ROOT),'--script',str(ROOT/'.github/hud-dev15-2/extract-assets.gd'),'--',str(carrier),str(ROOT)],work/'extract.log')
 native=json.loads((ROOT/'.github/hud-dev15-2/native-input.json').read_text())
 ...
 expected=json.loads((ROOT/'.github/hud-dev15-2/shell-input.json').read_text())
 ...
 (work/'build.json').write_text(json.dumps({'version':'1.0.0-dev.15.2','source_commit':os.environ.get('GITHUB_SHA'),'files':files,'save_path':'user://ever_deeper_dev_run_v3.sav','native_carrier':identity(carrier),'native_files_unchanged':native,'physical_iphone_verified':False},indent=2)+'\n')
