...
const RANGE_END = Number(process.env.CAPTURE_END || 25);
if (!(RANGE_START >= 1 && RANGE_END >= RANGE_START && RANGE_END <= 25)) throw new Error("Invalid capture range");
...
function injectSuiteArgument(html) {
  ...
  config.args = [...suiteArgs, SUITE_ARG, "--surface-capture", `--capture-start=${RANGE_START}`, `--capture-end=${RANGE_END}`];
  ...
  try {
    ...
    while (!completed) {
      ...
      if (marker.startsWith(`${MARKER_PREFIX}READY`)) {
        ...
        process.stdout.write(`CAPTURED ${index + RANGE_START - 1}/25 ${state}\n`);
