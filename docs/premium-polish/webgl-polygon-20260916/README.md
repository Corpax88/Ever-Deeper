...
actual framebuffer and error checks. Workflow 35077842852 completed the controlled comparison: control failed with
one observed buffer-target conflict and two native WebGL warnings; candidate
passed the same two fixtures with no graphics errors. `browser-comparison.json`
binds the actual package hashes, framebuffer observations, console and PNG hashes.
Both still pairs have localized differences near the animated pickaxe; these are
not whole-game pixel equality. Independent visual review found no geometry or
text corruption in those fixtures. The native 15-pair equality evidence remains
separate. Full active-hazard browser cycles and the combined UI/touch package gate
are still open. The repair is adopted into the continued work branch, not deployed.
