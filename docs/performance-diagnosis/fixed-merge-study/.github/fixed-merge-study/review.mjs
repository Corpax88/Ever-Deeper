...
async function width(value){await command('lightmode',{mode:value===0?'baseline':'merged'});await delay(650);check('variant-'+value,(await state()).light_cost.mode===(value===0?'baseline':'merged'));const merge=(await state()).light_cost.merge;check('field-state-'+value,merge.ready&&merge.active===(value===1)&&merge.field_enabled===(value===1));}
async function parity(label,retain=false){
 ...
 for(const [i,shot] of shots.entries()){
  ...
  if(shot.w===0)check('restored-'+label,max===0,row);
  // A cached field is resampled/quantized: record every difference; no visual acceptance inferred from this metric.
 }
}
...
try{
 ...
 for(const mine of ['emberMine','mossMine','moonMine','starMine']){
  ...
  await command('lightmode',{mode:'baseline'});await wait('field ready',s=>s?.light_cost?.merge?.ready);await delay(1000);await command('freeze');
 ...
 await command('lightmode',{mode:'baseline'});await wait('field ready',s=>s?.light_cost?.merge?.ready);
 ...
 fs.writeFileSync(path.join(output,'report.json'),JSON.stringify({passed:!failed,error:failed,source_commit:process.env.GITHUB_SHA,baseline_source:'c63aabd3e3e5120579285ce6e8b0b59a75f2727a',version,files,browser:browserName,repeat:process.env.REPEAT,runtime,checks,windows,pairs,captures,physical_iphone_verified:false,scope:'QA fixed-light precomposition; audited occupancy revision in both modes; native resolution. Quantization differences recorded and require visual review. No release.'},null,2));
