...
## Mossvein D2 corroboration

Run 34163220686 succeeded. All seven fixed-dimension captures inspected.
See mossvein-lighting-effects.json. Baseline/restored states: 2.74–2.81 native FPS;
shadows off: 3.95; all seven lights off: 16.62; root-world light mask zero with all
seven lights enabled: 15.11. Four lights cast shadows here, versus two in the hub.
The expensive native root-world lighting work is common to both reported areas.
Removing shadows helps more here (~44%) than the hub (~9%), but disabling world
lighting changes approved appearance and is not a candidate for publication.

## Hidden floor overdraw experiment (test branch only)

Source inspection found a full-size background rectangle immediately covered by
an opaque tiled floor in both world renderers. All seven assets/*/floor.png source
images are RGB (opaque). The diagnostic metadata qa_skip_covered_floor suppresses
that covered rectangle; hub retains four background border strips outside its
floor. Ordinary gameplay keeps the original commands (metadata defaults false).
No assets, lights, resolution or game behavior intentionally changed.

The first exact pixel gate (run 34163509604) correctly stopped both jobs: pause
alone leaves always-processing achievement/guide UI running, and the hub elevator
pulse reads wall-clock time directly during redraw. Difference images localize
the changes to those animated elements. That run is NOT a passing parity result.
The repaired gate temporarily disables descendant process callbacks, fixes the
hub draw timestamp, and compares before -> unchanged control -> candidate ->
restored at 2328x1260. It retains strict full-image byte equality; no changed-pixel
mask or tolerance is used. All callback states/time overrides are restored before
performance sampling. This is diagnostic instrumentation, not release code.

## Verified covered-floor result — 2026-09-07

Run 34163890969: both matrix jobs succeeded. Full provenance and artifact digests
are in lighting-runs.json. Covered-floor-hub.json and covered-floor-mossvein.json
record the results (actual filenames are lowercase).

Strict pixel gate passed in BOTH areas: baseline, unchanged control, candidate
and restored images have identical raw image bytes at 2328x1260. No tolerance,
masked pixels, resolution reduction, light removal or shadow removal was used.
All normal visual processing resumes before the FPS measurements.

Within-run native software results (do not compare raw FPS between runners):
- Hub: baseline 2.207, candidate 2.768, restored 2.225 FPS: about +25%.
  p95 frame time: 480.9 -> 385.9 -> 475.7 ms; 14 lights and 2 shadows retained.
- Mossvein D2: baseline 1.720, candidate 2.142, restored 1.721 FPS: about +25%.
  p95 frame time: 618.3 -> 491.9 -> 617.8 ms; 7 lights and 4 shadows retained.

This identifies a visually lossless optimization candidate for the two measured
scenes. It does not explain the delayed iPhone drop or predict device FPS gain.
The candidate remains a metadata-gated test-branch experiment; production drawing
still takes the original path. Do not merge the temporary QA clock/process hooks
as a finished release fix. A clean production change should remove the covered
background commands, retain hub border coverage, and verify all affected depth
profiles and current gameplay gates before a reviewed DEV publication.
