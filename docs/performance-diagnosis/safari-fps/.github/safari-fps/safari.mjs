...
const bridge=`<script>(()=>{
 let busy=false,gl=null,info=null;
 const send=(kind,data)=>fetch('/__error',{method:'POST',body:JSON.stringify({kind,data,at:Date.now()})}).catch(()=>{});
 ...
 window.addEventListener('webglcontextlost',e=>{
  e.preventDefault();const attempts=Number(sessionStorage.getItem('qa-context-reloads')||0);send('context-lost',{message:e.statusMessage,reloads:attempts,info});
 ...
 setInterval(async()=>{if(busy)return;busy=true;try{
  await fetch('/__state',{method:'POST',body:JSON.stringify({state:window.DEV14_STATE||null,ua:navigator.userAgent,width:innerWidth,height:innerHeight,dpr:devicePixelRatio,renderer:info?.renderer,context_lost:gl?.isContextLost(),canvas:gl?[gl.canvas.width,gl.canvas.height]:null})});
