...
func _run_landscape_qa() -> void :
	...
	for boundary_value in Array(surface_routes.boundaries):
		...
		assert (bool(boundary.generator_visual_present) and generator_rect.size == Vector2(360, 360))
		assert (is_equal_approx(generator_rect.get_center().x, anchor.x))
		assert (is_equal_approx(generator_rect.end.y, anchor.y + 69.0))
		assert (not generator_sprite.texture is AtlasTexture)
		var arch_paths: Dictionary = {"moonglass": "res://assets/surface/v2/moonglass-port.png", "emberdeep": "res://assets/surface/v3/ember-port.png", "starfall": "res://assets/surface/v3/star-port.png"}
		assert (generator_sprite.texture.resource_path == arch_paths[boundary_id])
		assert (bool(transition_debug.seam_mode) and bool(transition_debug.external_arch_mode))
		...
		assert (bool(boundary.player_occlusion_split) and Rect2(boundary.portal_arch_rect) == generator_rect)
		var front_sprite: Sprite2D = generator.front
		assert (front_sprite.texture == generator_sprite.texture)
		assert (not bool(generator_sprite.material.get_shader_parameter("front_layer")))
		assert (bool(front_sprite.material.get_shader_parameter("front_layer")))
		assert (generator_sprite.z_index < crossing_depth and front_sprite.z_index > crossing_depth)
