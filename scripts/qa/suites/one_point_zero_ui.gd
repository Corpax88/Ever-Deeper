...
func run() -> void:
	...
	var materials: Dictionary = _row("deep_alloy", _snapshot())
	_check(int(materials.get("owned", -1)) == 199 and int(materials.get("required", 0)) == 200, "Workshop counter shows 199 of exact 200")
	_check(not bool(materials.get("ready", true)), "Workshop not ready one material early")
	RunState.deliver_workshop_material("tool_forge", "deep_alloy", 100)
	materials = _row("deep_alloy", _snapshot())
	_check(int(materials.get("owned", -1)) == 199 and int(materials.get("delivered", 0)) == 100, "Delivered materials remain counted after leaving pouch")
	RunState.add_resource("deep_alloy", 1, true)
	var complete: Dictionary = _snapshot()
	_check(bool(_row("deep_alloy", complete).get("ready", false)), "Final pickup immediately marks collected cost ready")
	_check(String(_rendered("resource:deep_alloy", complete).get("text", "")) == "200 / 200", "Visible row updates before a guide-timer frame")
	RunState.deliver_workshop_material("tool_forge", "deep_alloy", 100)
