...
var outfit_surface: ShaderMaterial
var applied_outfit: Color = Color(-1, -1, -1)
var outfit_recolored: bool = false
...
func configure(candidate: String, pose_only: bool = false) -> bool:
	...
	if material_view in ["baked_response", "baked_shader_control"]:
		...
		surface.set_shader_parameter("cloth_mask", _texture(candidate.path_join("cloth.png")))
		outfit_surface = surface
...
func set_outfit(color: Color, recolored: bool) -> void:
	if outfit_surface == null or (color == applied_outfit and recolored == outfit_recolored): return
	applied_outfit = color
	outfit_recolored = recolored
	outfit_surface.set_shader_parameter("cloth_color", color)
	outfit_surface.set_shader_parameter("recolor", 1.0 if recolored else 0.0)
